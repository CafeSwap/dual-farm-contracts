...
const CafeSwapSafeTransfer = artifacts.require('CafeSwapSafeTransfer');
...
contract('masterChefv2', ([alice, bob, carol, dev, minter, referrer, referred]) => {
    ...
    context('masterChef test', () => {
        beforeEach(async () => {
            ...
            this.cafeswap = await CafeSwapSafeTransfer.new(this.mocha.address,{from: minter})

            await this.mocha.setWhiteListAccount(
                this.cafeswap.address,
                true,
                { from: minter }
            )
        });
        ...
        it('should give out mochas only after farming time', async () => {
            ...
            await this.cafeswap.transferOwnership(this.masterChef.address,{from:minter});
        ...
        it('should give proper mochas allocation to each pool and deduct 5% fee on rewards distribution ', async () => {
            this.masterChef = await masterChefV2.new(this.mocha.address,minter,minter,'1000','200', this.cafeswap.address, {from: minter});
        ...
        it('should distribute mochas properly for each staker', async () =>{
            ...
            this.masterChef = await masterChefV2.new(this.mocha.address, minter, minter,'1000', '300', this.cafeswap.address, { from: minter });
            await this.mocha.setWhiteListAccount(this.masterChef.address, true, { from: minter })
        ...
        it('should distribute only 449k mochas  after it brewPerBlock becomes 0(no reward distribution)', async () => {
            this.masterChef = await masterChefV2.new(this.mocha.address, minter, minter,web3.utils.toWei('23000'), '400', this.cafeswap.address,  { from: minter });
            await this.mocha.setWhiteListAccount(this.masterChef.address,true,{from:minter})
            ...
            assert.equal((await this.masterChef.totalSupply()).valueOf(), web3.utils.toWei('449000'));
            assert.equal((await this.mocha.balanceOf(carol)).valueOf(), '0');
            assert.equal((await this.mocha.balanceOf(minter)).valueOf(),  web3.utils.toWei('44900'));
        ...
        it('should allow emergency withdraw', async () => {
            this.masterChef = await masterChefV2.new(this.mocha.address, minter,minter, '100', '100', this.cafeswap.address, { from: alice });
            await this.mocha.setWhiteListAccount(this.masterChef.address,true,{from:minter})
        ...
        it('should set correct state variables', async () => {
            this.masterChef = await masterChefV2.new(this.mocha.address, dev,dev, '1000', '1000',     this.cafeswap.address,      { from: alice });
            await this.mocha.transferOwnership(this.masterChef.address, { from: minter });
            await this.cafeswap.transferOwnership(this.masterChef.address,{from:minter});
        ...
        it('should allow feeAddress and only feeAddress to update feeAddress', async () => {
            this.masterChef = await masterChefV2.new(this.mocha.address, dev,minter, '1000', '1000',this.cafeswap.address,      { from: alice });
      ...
      it('It should take a commission if a referral is set once, and all other deposits are using address(0)', async () => {
        ...
        // 1000/10 = 100 To Dev, 1000-100 = 900 to Bob
        console.log(Number(await this.mocha.balanceOf(this.cafeswap.address)))
        console.log(Number(await this.mocha.balanceOf(bob)))
        console.log(Number(await this.mocha.balanceOf(alice)))
        console.log(Number(await this.mocha.balanceOf(minter)))
        ...
        assert.equal((await this.mocha.balanceOf(alice)).valueOf(), '45');
        assert.equal((await this.mocha.totalSupply()).valueOf(), '5000');
        assert.equal((await this.brewReferral.referralsCount(alice)).valueOf(), '1')
        assert.equal((await this.brewReferral.totalReferralCommissions(alice)).valueOf(), '45')

        await this.masterChef.deposit(0, '100', alice, { from:  carol})

        assert.equal((await this.brewReferral.referralsCount(alice)).valueOf(), '2')
